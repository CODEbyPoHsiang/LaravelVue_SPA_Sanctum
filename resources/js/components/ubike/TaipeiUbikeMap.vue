...
<template>
  ...
  <p class="text-dark">站名地圖</p>
    ...
    <!-- 搜尋功能 -->
      <p class="text-dark">站名搜尋</p>

    <input type="text" v-model="keywords" placeholder="請輸入完整站名名稱" />
  ...
  methods: {
    ...
    search() {
        ...
        if (this.keywords === response.data[0].sna ) {
          // this.OSMap.panTo(new L.LatLng(dist.latitude, dist.longitude)); // 直接平移
          this.OSMap.flyTo(new L.LatLng(response.data[0].lat, response.data[0].lng, 14)); // 飛越效果，數字為過程中縮放級數
